...
**`batchGetOrCreateBreakdowns(cards, onCardReady)`** — use this for bulk import flows (e.g. PDF import). Takes an array of `{id, vietnamese, english}` objects and a callback fired as each breakdown becomes available. Collapses N cache reads, N cache writes, and N flashcard updates into 1 each. For cache misses, calls the `batch-breakdown` Edge Function in sequential chunks of 20 (not concurrently) to avoid rate limits — each chunk is one Claude call covering up to 20 words. Results are matched by array index, not by string key. `onCardReady(cardId, breakdown)` is called per card as each chunk completes.
